...
const ViewApplications = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const token = localStorage.getItem("accessToken");
      const url = `http://localhost:8000/api/company-applications/${id}/`;
      console.log("Updating application status, id:", id, "newStatus:", newStatus, "url:", url, "token:", token ? "Present" : "Missing");
      if (!token) {
      ...
      console.log("Application status updated, response:", response.data, "status:", response.status, "url:", url);
      setApplications((prevApplications) =>
        prevApplications.map((app) =>
          app.id === id ? { ...app, status: newStatus } : app
        )
      );
      const message = response.data.message || `Application ${newStatus.toLowerCase()} successfully!`;
      setSuccess(`${message} Notification sent to applicant.`);
      setError("");
      setTimeout(() => setSuccess(""), 3000);
    } catch (err) {
      console.error("Failed to update status, error:", err.message, "response:", err.response?.data, "status:", err.response?.status, "url:", url);
